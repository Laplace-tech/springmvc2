 ...
 * - View 에 도메인 객체가 아닌 View DTO를 전달하도록 변경
 */
@Slf4j
@Controller
...
public class ItemController {
	...
	public String showAllItems(Model model) {
		...
		List<ItemViewDto> viewDtos = itemList.stream()
										.map(ItemMapper::toViewDto)
										.collect(Collectors.toList());
		model.addAttribute("itemList", viewDtos);
		return "items/items";
	}
	...
	public String showItemDetail(@PathVariable("itemId") Long itemId, Model model) {
		...
		ItemViewDto viewDto = ItemMapper.toViewDto(item);
		model.addAttribute("item", viewDto);
		return "items/item";
	}
